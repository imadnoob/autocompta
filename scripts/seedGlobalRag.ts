...
async function seedGlobalBase() {
    console.log(`Checking existing global entries...`);

    // Check if already seeded globally (user_id IS NULL)
    ...
    if (countError) {
        console.error('Error checking base:', countError.message);
        return;
    }

    if (count && count > 0) {
        console.log(`Global knowledge base already seeded with ${count} entries. Skipping.`);
        return;
    }

    let insertedCount = 0;

    console.log(`Starting generation for ${TEXTBOOK_ENTRIES.length} entries...`);
    ...
    for (let i = 0; i < TEXTBOOK_ENTRIES.length; i += 5) {
        ...
        const rows = await Promise.all(batch.map(async (entry) => {
            try {
                ...
                console.error(`Failed to embed: ${entry.situation} — ${e.message}`);
                return null;
            }
        }));
        ...
        if (validRows.length > 0) {
            ...
            if (insertError) {
                console.error(`Insert error on batch ${i}: ${insertError.message}`);
            } else {
                insertedCount += validRows.length;
                console.log(`Inserted ${insertedCount} / ${TEXTBOOK_ENTRIES.length} entries...`);
            }
        }
    ...
    console.log(`Successfully seeded ${insertedCount} / ${TEXTBOOK_ENTRIES.length} global textbook entries.`);
}

seedGlobalBase().catch(console.error);
